		...
		public void SetTypeAndFlags(MySqlDbType type, ColumnFlags flags)
		{
			...
			if (this.IsBlob)
			{
				if (this.IsBinary && this.driver.Settings.TreatBlobsAsUTF8)
				{
					...
					{
						if (blobAsUTF8IncludeRegex == null && blobAsUTF8ExcludeRegex != null && !blobAsUTF8ExcludeRegex.IsMatch(this.ColumnName))
						{
							flag = true;
						}
					}
			...
			if (this.driver.Settings.RespectBinaryFlags)
			{
				...
				{
					if (type == MySqlDbType.VarChar || type == MySqlDbType.VarString)
					{
