	...
	public sealed class MySqlCommand : DbCommand, ICloneable, IDisposable
	{
		...
		public new MySqlDataReader ExecuteReader(CommandBehavior behavior)
		{
			...
			lock (driver)
			{
				...
				{
					if (this.CommandType == CommandType.Text && text.IndexOf(" ") == -1 && !MySqlCommand.SingleWordKeywords.Contains(text.ToUpper()))
					{
						text = "call " + text;
					}
				}
		...
		{
			return Task.Factory.StartNew<int>(() => this.ExecuteNonQuery());
		}
		...
		{
			return Task.Factory.StartNew<MySqlDataReader>(() => this.ExecuteReader(behavior));
		}
		public new Task<object> ExecuteScalarAsync()
		{
			return Task.Factory.StartNew<object>(() => this.ExecuteScalar());
		}
