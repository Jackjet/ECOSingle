...
{
	public class MinuteDataProcess
	{
		...
		private static MinuteDataProcess mdp_instance;
		...
		public static MinuteDataProcess GetInstance()
		{
			if (MinuteDataProcess.mdp_instance == null)
			{
				MinuteDataProcess.mdp_instance = new MinuteDataProcess();
				MinuteDataProcess.mdp_instance.Start();
			}
			return MinuteDataProcess.mdp_instance;
		}
		public void Dispose()
		{
		}
		private MinuteDataProcess()
		{
		...
		private void WorkThread(object state)
		{
					...
					if (num == 1)
					{
						...
						if (list != null)
						{
							...
							if (text.Equals("POWERSTART"))
							{
								...
								this.preparedatabase(dateTime, false);
							}
							else
							{
								if (text.Equals("PDSTART"))
								{
									...
									this.i_trace = random.Next(111111, 999999);
									DebugCenter.GetInstance().appendToFile("<><><> Begin update Power Consumption Data (" + this.i_trace + ")");
									this.update_pd = true;
									this.preparedatabase(dateTime, true);
								}
								else
								{
									...
									else
									{
										if (text.Equals("END"))
										{
											try
											{
												...
												DBCacheStatus.MinuteDataDBReady = true;
												DebugCenter.GetInstance().appendToFile("<><><> Finish update Power Consumption Data (" + this.i_trace + ")");
												continue;
											}
											catch (Exception)
											{
			...
			{
				DebugCenter.GetInstance().appendToFile("Fatal Error : MinuteDataProcess Thread was aborted. \r\n" + ex3.Message);
			}
			finally
			{
				try
		...
		private void preparedatabase(DateTime dt_inserttime, bool b_pd)
		{
			try
			{
				this.sb_dd = new StringBuilder();
