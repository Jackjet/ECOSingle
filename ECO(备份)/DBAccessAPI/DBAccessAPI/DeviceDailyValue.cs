...
namespace DBAccessAPI
{
	public class DeviceDailyValue
	{
		...
		public static DataTable GetAllDevicePD()
		{
			...
			{
				commandText = "select * from device_data_daily" + DateTime.Now.ToString("yyyyMMdd");
				try
				{
			...
			commandText = "select * from device_data_daily ";
			try
			{
		...
		public static double GetLastDailyPD(DataTable dt_devicePD, int i_did)
		{
			try
			{
				if (dt_devicePD != null)
				{
					DataRow[] array = dt_devicePD.Select(" device_id = " + i_did);
