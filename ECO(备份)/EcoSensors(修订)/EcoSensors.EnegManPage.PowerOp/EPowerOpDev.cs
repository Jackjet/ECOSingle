...
{
	public class EPowerOpDev : UserControl
	{
		...
		public EPowerOpDev()
		{
			...
			EcoGlobalVar.gl_PowerOPCtrl = this;
		...
		public void pageInit(int devID, string devName, EPowerOp parent)
		{
			this.endTimer();
		...
		private void theout(object source, ElapsedEventArgs e)
		{
			...
			{
				base.Invoke(new EPowerOpDev.devRefresh(this.deviceTimerRefresh));
				return;
			}
		...
		private void except_devnotExist(string devName)
		{
			...
			EPowerOpDev.PostMessage(this.m_parent.Handle, 63000u, 0, "");
		}
		private void initDataTable()
		{
			...
			dataTable = (DataTable)this.dgvOutletInfo.DataSource;
			if (dataTable != null)
			{
				dataTable.Clear();
			}
			dataTable = (DataTable)this.dgvDeviceSensorInfo.DataSource;
			if (dataTable != null)
			{
				dataTable.Clear();
			}
			dataTable = (DataTable)this.dgvBankInfo.DataSource;
			if (dataTable != null)
			{
				dataTable.Clear();
			}
		...
		private void deviceTimerRefresh()
		{
			...
			try
			{
				if (this.m_devID < 0)
				{
					this.dTimer.Interval = 1000.0;
					return;
				}
				int num = this.m_parent.refreshTree();
				if (num == -1)
				{
					this.dTimer.Interval = 1000.0;
					return;
				...
				if (num == 1)
				{
					this.except_devnotExist("");
					this.dTimer.Interval = 10000.0;
					return;
				}
				string deviceId = this.m_devID.ToString();
				DataRow[] dataRows = ClientAPI.getDataRows(0, "device_id=" + this.m_devID + " and device_state=1", "");
				...
				int deviceData = this.getDeviceData(deviceId, dataRows);
				this.getOutletData(deviceId, false, deviceData);
				this.getDeviceSensorData(deviceId, deviceData);
				this.getBankData(deviceId, false, deviceData);
				this.getLineData(deviceId, false, deviceData);
			}
			catch (System.Exception ex)
			{
				System.Console.WriteLine("EPowerOPDev deviceTimerRefresh error!" + ex.Message);
			}
			this.dTimer.Interval = 10000.0;
		}
		...
		private int getDeviceData(string deviceId, DataRow[] drArr)
		{
			...
			for (int i = 0; i < drArr.Length; i++)
			{
				...
				this.dataTransPdu(dataRow, dataRow2);
		...
		private void dataTransPdu(DataRow srcdr, DataRow tardr)
		{
		...
		private void getBankData(string deviceId, bool initFlg, int devonline)
		{
			...
			{
				uint bankCtrlflg = DevAccessCfg.GetInstance().getDeviceModelConfig(this.m_ModelNm, this.m_FWVer).bankCtrlflg;
				for (int k = 0; k < this.dgvBankInfo.Rows.Count; k++)
				{
					DataGridViewRow dataGridViewRow2 = this.dgvBankInfo.Rows[k];
					if (((ulong)bankCtrlflg & (ulong)(1L << (k & 31))) != 0uL && EcoGlobalVar.flgEnablePower)
					{
		...
		private void pageControlInit()
		{
			...
			bool flag2 = true;
			if (deviceModelConfig.perlineReading == Constant.NO)
			{
				flag2 = false;
			}
			if (!flag2)
			...
			if (deviceModelConfig.perbankReading == 1)
			{
				this.labBankStatus.Hide();
				this.dgvBankInfo.Hide();
				return;
		...
		private void butOn_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				bool flag = devAccessAPI.TurnOnAllOutlets();
				if (flag)
				{
					int devonline = 0;
		...
		private void butOff_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				else
				{
					...
					flag = devAccessAPI.TurnOffAllOutlets();
					if (flag)
					{
		...
		private void dgvOutletInfo_CurrentCellDirtyStateChanged(object sender, System.EventArgs e)
		{
			if (this.dgvOutletInfo.IsCurrentCellDirty)
			{
				this.dgvOutletInfo.CommitEdit(DataGridViewDataErrorContexts.Commit);
			}
		}
					...
					if (e.RowIndex >= 0)
					{
						...
						else
						{
							if (this.dgvBankInfo.Columns[e.ColumnIndex].Name.Equals("bank_acton"))
							{
								...
								if (!devAccessAPI.TurnOnBank(System.Convert.ToInt32(value)))
								{
									EcoMessageBox.ShowError(EcoLanguage.getMsg(LangRes.OPfail, new string[0]));
									this.starTimer();
									return;
							...
							if (this.dgvBankInfo.Columns[e.ColumnIndex].Name.Equals("bank_actoff"))
							{
								...
								if (dataGridViewDisableButtonCell2.Enabled)
								{
									...
									else
									{
										...
										DevAccessAPI devAccessAPI2 = new DevAccessAPI(sNMPpara2);
										this.endTimer();
										...
										if (this.dgvBankInfo.Rows[e.RowIndex].Cells["bank_reboot"].Value.Equals("0"))
										{
											...
											if (!devAccessAPI2.TurnOffBank(System.Convert.ToInt32(value2)))
											{
												EcoMessageBox.ShowError(EcoLanguage.getMsg(LangRes.OPfail, new string[0]));
												this.starTimer();
												return;
										...
										else
										{
											...
											if (!devAccessAPI2.RebootBank(System.Convert.ToInt32(value2)))
											{
												EcoMessageBox.ShowError(EcoLanguage.getMsg(LangRes.OPfail, new string[0]));
												this.starTimer();
												return;
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(EPowerOpDev));
			...
			this.dgvDeviceSensorInfo.ColumnHeadersDefaultCellStyle = dataGridViewCellStyle;
			...
			this.dgvDeviceSensorInfo.DefaultCellStyle = dataGridViewCellStyle2;
			...
			this.dgvDeviceInfo.ColumnHeadersDefaultCellStyle = dataGridViewCellStyle3;
			...
			this.dgvDeviceInfo.DefaultCellStyle = dataGridViewCellStyle4;
			...
			this.dgvOutletInfo.ColumnHeadersDefaultCellStyle = dataGridViewCellStyle5;
			...
			this.dgvOutletInfo.DefaultCellStyle = dataGridViewCellStyle6;
			...
			this.dgvBankInfo.ColumnHeadersDefaultCellStyle = dataGridViewCellStyle7;
			...
			this.dgvBankInfo.DefaultCellStyle = dataGridViewCellStyle8;
