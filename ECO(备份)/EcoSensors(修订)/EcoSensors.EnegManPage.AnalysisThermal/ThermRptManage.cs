...
{
	public class ThermRptManage : UserControl
	{
		...
		public void pageInit()
		{
			...
			this.txtpath.Text = Sys_Para.GetThermalPath();
			if (this.txtpath.Text.Length == 0)
			{
				this.txtpath.Text = System.IO.Directory.GetCurrentDirectory() + "\\THReportFile\\";
			}
		...
		private void DataBind(string type)
		{
			...
			if (type != null)
			{
				if (type == "0")
				{
					...
					{
						"select id,Title,Writer,ReportTime,ReportPath from reportthermal where ReportTime between #",
						text,
						"# and #",
						text2,
				...
				if (type == "1")
				{
					str_sql = "select id,Title,Writer,ReportTime,ReportPath from reportthermal where title like '%" + str + "%' order by ReportTime desc";
					goto IL_D1;
				}
				if (type == "2")
				{
					str_sql = "select id,Title,Writer,ReportTime,ReportPath from reportthermal where writer like '%" + str + "%' order by ReportTime desc";
					goto IL_D1;
				}
			}
			str_sql = "select id,Title,Writer,ReportTime,ReportPath from reportthermal order by ReportTime desc";
			IL_D1:
		...
		private void btndel_Click(object sender, System.EventArgs e)
		{
			if (this.dgvwReport.SelectedRows.Count > 0)
			{
				if (EcoMessageBox.ShowWarning(EcoLanguage.getMsg(LangRes.RptMng_delCrm, new string[0]), MessageBoxButtons.OKCancel).Equals(DialogResult.OK))
				{
					...
					string sql = "delete from reportthermal where id=" + this.dgvwReport.SelectedRows[0].Cells[1].Value;
		...
		private void btnSave_Click(object sender, System.EventArgs e)
		{
			...
			int num = Sys_Para.UpdateThermalPath(this.txtpath.Text.Replace("'", "''"));
			if (num > 0)
			{
				EcoMessageBox.ShowInfo(EcoLanguage.getMsg(LangRes.OPsucc, new string[0]));
			}
